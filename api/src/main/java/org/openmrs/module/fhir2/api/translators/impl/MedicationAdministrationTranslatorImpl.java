...
public class MedicationAdministrationTranslatorImpl extends BaseReferenceHandlingTranslator implements MedicationAdministrationTranslator {
    ...
    public MedicationAdministration toFhirResource(@Nonnull FhirMedicationAdministration fhirMedicationAdministration) {
        ...
        medicationAdministration.setSubject(referenceTranslator.toFhirResource(fhirMedicationAdministration.getSubjectReference()));

        if (fhirMedicationAdministration.getPerformer() != null && !fhirMedicationAdministration.getPerformer().isEmpty()) {
            medicationAdministration.setPerformer(fhirMedicationAdministration.getPerformer().stream().map(performerTranslator::toFhirResource)
                    .filter(obj -> obj != null).collect(Collectors.toList()));
        }

        medicationAdministration.setRequest(referenceTranslator.toFhirResource(fhirMedicationAdministration.getRequestReference()));
        ...
        if (fhirMedicationAdministration.getNote() != null && !fhirMedicationAdministration.getNote().isEmpty()) {
            medicationAdministration.setNote(fhirMedicationAdministration.getNote().stream().map(annotationTranslator::toFhirResource)
                    .filter(obj -> obj != null).collect(Collectors.toList()));
        }
    ...
    public FhirMedicationAdministration toOpenmrsType(@Nonnull FhirMedicationAdministration existingFhirMedicationAdministration, @Nonnull MedicationAdministration medicationAdministration) {
        ...
        if (medicationAdministration.hasEffectiveDateTimeType()) {
            existingFhirMedicationAdministration.setEffectiveDateTime(medicationAdministration.getEffectiveDateTimeType().getValue());
        }

        if(medicationAdministration.hasStatus()) {
            existingFhirMedicationAdministration.setStatus(statusTranslator.toOpenmrsType(medicationAdministration.getStatus()));
        }

        existingFhirMedicationAdministration.setSubjectReference(referenceTranslator.toOpenmrsType(medicationAdministration.getSubject()));
        if (medicationAdministration.getPerformer() != null && !medicationAdministration.getPerformer().isEmpty()) {
            existingFhirMedicationAdministration.setPerformer(medicationAdministration.getPerformer().stream().map(performerTranslator::toOpenmrsType)
                    .filter(obj -> obj != null).collect(Collectors.toSet()));
        }

        existingFhirMedicationAdministration.setRequestReference(referenceTranslator.toOpenmrsType(medicationAdministration.getRequest()));
        ...
        if (medicationAdministration.getNote() != null && !medicationAdministration.getNote().isEmpty()) {
            existingFhirMedicationAdministration.setNote(medicationAdministration.getNote().stream().map(annotationTranslator::toOpenmrsType)
                    .filter(obj -> obj != null).collect(Collectors.toSet()));
        }
